...
const Tasks = ({ list, onEditTitle, onAddTask }) => {
    ...
    return (
        <div className="tasks">
            <h2 className="tasks__title">{list.name} <img onClick={editTitle} src={EditSVG} alt="Edit icon" /></h2>
            <div className="tasks__items">

                {/* { !list.tasks.length && <h2>Задачи отсутсвуют<h2/> } */}
                {/* <div>{ !list.tasks.length && <h2>Задачи отсутсвуют<h2/> }</div> */}

                {
                    list.tasks.map(task => 
                        <div key={task.id} className="tasks__items-row">
                            <div className="checkbox">
                                <input id={`task-${task.id}`} type="text" />
